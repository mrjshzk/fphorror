...
func _physics_process(delta: float) -> void:
	if is_holding:
		if object != null and object == self.get_collider():
			print(object.get_parent_node_3d().get_node("%Joint").rotation * get_speed_add())
			object.apply_force(object.get_parent_node_3d().get_node("%Joint").global_position * get_speed_add())
